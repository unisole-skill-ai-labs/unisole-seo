      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 sm:pt-32 pb-20 space-y-16">
        ...
        <header className="max-w-3xl space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 text-xs font-mono">
            ...
            <span>Academic Framework 2026</span>
          </div>
          ...
          <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed max-w-xl">
            Four specialized academic tracks taking students from foundational programming to live MLOps laboratory deployments.
          </p>

          {/* Quick Group Switcher Tab Bar */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 pt-4" role="tablist">
            {GROUPS_DATA.map((g) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-start p-3 sm:p-4 rounded-xl border transition-all duration-150 text-left cursor-pointer ${
                    isActive 
                ...
                >
                  <span className={`text-[9px] font-mono uppercase tracking-wider block mb-1 ${isActive ? 'text-zinc-400 dark:text-zinc-600' : 'text-zinc-400'}`}>{g.badge}</span>
        ...
        <section className="space-y-6" id="group-content">
          ...
          <div className="minimal-card p-6 sm:p-8 space-y-3">
            <div className="flex flex-wrap items-center gap-2.5">
              <span className="px-2 py-0.5 text-[9px] font-mono bg-zinc-100 dark:bg-zinc-800 rounded uppercase tracking-wider text-zinc-700 dark:text-zinc-300">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white tracking-tight">{currentGroupData.title}</h2>
            <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 max-w-2xl leading-relaxed">{currentGroupData.tagline}</p>
          </div>
          ...
          <div className="space-y-3">
            {currentGroupData.pathways.map((pathway) => {
              ...
              return (
                <article
                  key={pathway.id}
                  className={`minimal-card overflow-hidden transition-all duration-200 ${
                    isOpen 
                ...
                >
                  ...
                  {isOpen && (
                    <div className="px-5 sm:px-6 pb-6 pt-2 border-t border-slate-100 dark:border-slate-800 space-y-6 animate-in fade-in duration-200">
                      ...
                      <div className="space-y-3 pt-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                          {pathway.modules.map((mod) => (
                            <div key={mod.num} className="border border-zinc-200/80 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/40 rounded-xl p-4 space-y-3">
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-mono font-bold text-zinc-700 bg-zinc-200/60 dark:bg-zinc-800 dark:text-zinc-300 w-5 h-5 rounded flex items-center justify-center">
                                  {mod.num}
                                </span>
                                <h5 className="text-xs sm:text-sm font-bold text-zinc-900 dark:text-white leading-tight">{mod.title}</h5>
                              </div>
                              ...
                              {mod.practical && (
                                <div className="mt-2.5 pt-2 border-t border-zinc-200/60 dark:border-zinc-800 text-xs">
                                  <span className="font-semibold text-zinc-700 dark:text-zinc-300 block text-[11px]">Lab Task:</span>
                              ...
                              {mod.pipeline && (
                                <div className="mt-2.5 pt-2 border-t border-zinc-200/60 dark:border-zinc-800 text-xs">
                                  <span className="font-semibold text-zinc-700 dark:text-zinc-300 block text-[11px]">Architecture:</span>
                      ...
                      {pathway.capstone && (
                        <div className="border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/40 rounded-xl p-5 space-y-3">
                          <div className="flex flex-wrap items-center justify-between border-b border-zinc-200/80 dark:border-zinc-800 pb-2.5 gap-2">
                            ...
                            <h4 className="text-xs sm:text-sm font-bold text-zinc-900 dark:text-white">{pathway.capstone.title}</h4>
                          </div>

                          {pathway.capstone.flow && (
                            <div className="space-y-1.5">
                              <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-400 block">Workflow:</span>
                              <div className="flex flex-wrap items-center gap-2">
                          ...
                          {pathway.capstone.outputs && (
                            <div className="space-y-1.5 pt-1">
                              <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-400 block">Deliverables:</span>
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                {pathway.capstone.outputs.map((out, oIdx) => (
                                  <div key={oIdx} className="flex items-center gap-2 text-xs text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-900 p-2 rounded-lg border border-zinc-200 dark:border-zinc-800">
                      ...
                      {/* Action buttons */}
                      <div className="pt-3 border-t border-zinc-100 dark:border-zinc-800 flex flex-wrap items-center gap-2.5">
                        <button
                          type="button"
